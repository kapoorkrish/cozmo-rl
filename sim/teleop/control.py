...
def get_actuator_id(m, name):
    i = mujoco.mj_name2id(m, mujoco.mjtObj.mjOBJ_ACTUATOR, name)
    if i < 0:
        raise KeyError(f"No actuator named {name}")
    return i


def ramp(cur, tgt, dt, scale):
...
class Control:
    def __init__(self, m, d):
        ...
        self.a_lwheel = get_actuator_id(m, "left_front_motor")
        self.a_rwheel = get_actuator_id(m, "right_front_motor")
        self.a_head = get_actuator_id(m, "head_motor")
        self.a_lift = get_actuator_id(m, "lift_motor")
    ...
    def apply(self, held, dt):
        ...
        self.v_cur = ramp(self.v_cur, v_target, dt, scale)
        self.w_cur = ramp(self.w_cur, w_target, dt, scale)
